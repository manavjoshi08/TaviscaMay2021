...
let ITEmps = arr.filter((v,i)=> {
    return v.DeptName == 'IT';
});

let HREmps = arr.filter((vi) => {
    return vi.DeptName == 'HR';
});

let TechEmps = arr.filter((vi) => {
    return vi.DeptName == 'Tech';
});

let ManagementEmps = arr.filter((vi) => {
    return vi.DeptName == 'Management';
});

let BoardEmps = arr.filter((vi) => {
    return vi.DeptName == 'Board';
});

let DirectorEmps = arr.filter((vi) => {
    return vi.DeptName == 'Director';
});

//List of employees dept wise
ITEmps.forEach((v, i) => {
    console.log(`The IT employees names are ${v.EmpName}`);
});

TechEmps.forEach((v, i) => {
    console.log(`The Tech employee names are ${v.EmpName}`);
});
